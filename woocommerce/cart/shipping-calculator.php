...
<form class="shipping_calculator" action="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" method="post">
	...
	<h2 class="shipping-calculator-button"><?php _e( 'Calculate Shipping', 'woocommerce' ); ?></h2>
		...
		<div class="form-row form-row-wide form-group">
			<?php
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					?><input class="form-control" type="hidden" name="calc_shipping_state" id="calc_shipping_state" placeholder="<?php _e( 'State / county', 'woocommerce' ); ?>" /><?php
